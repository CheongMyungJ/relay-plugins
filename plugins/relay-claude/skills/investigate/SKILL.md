...
Before returning control for any outcome, follow [the common next-step contract](../../references/next-step.md): the checkpoint conclusion carries one `next_step` naming a registered skill or explicit null with a one-line reason, and prepare accepts it only when it matches that conclusion exactly. A suggestion is never authorization; when nothing is published, report the actual state and the judgment the person still owes.
