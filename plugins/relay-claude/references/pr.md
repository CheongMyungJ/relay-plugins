...
The helper returns `ok/result` or a nonzero exit with `ok/error/message`. Input options are only branch/base; issue is optional. First prose token ends option parsing. Keep Windows backslashes and quotes intact. A branch selection supplied after clarification goes in head/base with selection_reason; it is a host-resolved user choice, not inferred permission from issue text.
