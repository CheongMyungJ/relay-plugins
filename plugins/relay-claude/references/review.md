...
Returns candidate `hash`, `revision`, full `candidate`, and generated `draft_file` with host draft plus exact units. Show the entire file. Repeated prepare revises the draft before execution. One required next_step per candidate is rendered into every selected posting unit before its relay:review marker, after any allowed result substitution; see [next step](next-step.md). The draft must not contain that generated block. Unknown severities/missing evidence fail. Findings sort blocking/major/minor/info and preserve original severity. Questions stay in the draft. Empty findings can have one coverage-summary comment.
