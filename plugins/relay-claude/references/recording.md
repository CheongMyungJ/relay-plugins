...
inspect returns a `kb` field: for implement the entries for the selected proof's backtick paths and IDs (at most 8,000 characters, with a cursor), for document stages the counts and how to query, and null without a knowledge base. With a KB present, prepare of intent, spec, plan, brief and implementation bodies requires the `## 참조한 KB 항목` section and rejects unknown, superseded or deleted IDs, absorbed entries cited without their current pointer, and an empty lookup line without its paths or terms; see [kb](kb.md).
